...
def product_list(request, slug=None):
    ...
    return render(request, "store/collection.html", {"page_obj": page, "category": category, "sizes": Size.objects.all(), "colors": Color.objects.all(), "all_categories": Category.objects.filter(is_active=True), "selected_sizes": size, "selected_colors": color, "active_filter_count": len(size)+len(color)+bool(request.GET.get("available"))+bool(request.GET.get("price_min"))+bool(request.GET.get("price_max"))})
